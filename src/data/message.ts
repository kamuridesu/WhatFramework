import { GroupParticipant, WAMessage, downloadMediaMessage } from "baileys";
import {
  IBot,
  Media,
  IMessage,
  ...
  get isGroupOwner(): Promise<boolean> {
    return (async () => {
      if (this.group != undefined)
        return (await this.group.members)
          .filter((x) => x.isSuperAdmin)
          .map((x) => x.id)
          .includes(this.jid);
      return false;
    })();
...
export class QuotedMessageParsed implements IQuotedMessageParsed {
  ...
  constructor(unparsedQuotedMessage: IQuotedMessageUnparsed, chatJid: string) {
    ...
    this.author = new Author(
      unparsedQuotedMessage.participant,
      "unknown",
      chatJid,
      false,
      false,
...
export class Group implements IGroup {
  ...
  get name(): Promise<string> {
    return (async () => {
      ...
      const x = (await this.bot.connection!.groupMetadata(this.context))
        .subject;
      this.__name = x;
      return x;
    })();
  ...
  get description(): Promise<string | undefined> {
    return (async () => {
      ...
      const desc = (await this.bot.connection!.groupMetadata(this.context))
        .desc;
  ...
  get members(): Promise<GroupParticipant[]> {
    return (async () => {
      ...
      const members = (await this.bot.connection!.groupMetadata(this.context))
        .participants;
  ...
  get admins(): Promise<GroupParticipant[]> {
    return (async () => {
      ...
      const a = (
        await this.bot.connection!.groupMetadata(this.context)
      ).participants.filter(
        (element) =>
          element.admin === "admin" || element.admin === "superadmin",
      );
      this.__admins = a;
      return a;
    })();
  ...
  get locked(): Promise<boolean> {
    return (async () => {
      const announce = (await this.bot.connection!.groupMetadata(this.context))
        .announce;
      return announce !== undefined
        ? JSON.parse(JSON.stringify(announce).replace(/"/g, ""))
        : false;
    })();
  }
  ...
  get botIsAdmin(): Promise<boolean> {
    return (async () => {
      ...
      const b = (await this.admins)
        .map((x) => x.id)
        .includes(this.bot.botNumber!);
      this.__botIsAdmin = b;
      return b;
    })();
  }
...
export class Message implements IMessage {
  ...
  edit(text: string, options?: {}): Promise<IMessage | undefined> {
    return this.bot.sendTextMessage(this, text, {
      edit: this.originalMessage.key,
      ...options,
    });
  }

  async downloadMedia(): Promise<Media> {
    const messageMedia = this.hasQuotedMessage
      ? JSON.parse(JSON.stringify(this.originalMessage).replace("quotedM", "m"))
